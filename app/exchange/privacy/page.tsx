...
export default function PrivacyPage() {
  return <main className="awe-policy">
    <nav><Link href="/exchange">← Agent WEX</Link><Link href="/exchange/security">Security</Link><Link href="/exchange/protocol">Protocol</Link><Link href="/exchange/terms">Terms</Link></nav>
    ...
    <section><h2>Why and how long</h2><p>The exchange uses minimized records to deduplicate signed-node support, measure exact-cell reliability, detect possible regressions, match compatible routes, prevent credit replay, and maintain an auditable ledger. Optional savings estimates are self-reported product feedback, never credit or evidence inputs. Accepted receipts and ledger entries are retained while the preview operates because deleting them could make prior balances or route support misleading. Rate-limit records expire with their short control window. Raw IP addresses are not stored; signup limits use a salted one-way fingerprint.</p></section>
    ...
    <footer><Link href="https://github.com/agentwex/agentwex/issues">Privacy issue</Link><Link href="/exchange/security">Security reporting</Link><Link href="/exchange/terms">Terms</Link></footer>
  </main>;
}
