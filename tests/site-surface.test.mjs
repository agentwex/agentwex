...
test("the standalone site makes prevention, failure value, and scope explicit", async () => {
  ...
  assert.match(page, /research will never start without your approval/i);
  ...
  assert.match(page, /Not live yet/);
  assert.match(page, /not currently accepting community bounty funds or paid claims/i);
  assert.doesNotMatch([protocol, skill].join("\n"), /\b(?:pay|pays|paid|payment|charge|costs?|spend|spends|spent|purchase|purchased)\b/i);
  assert.match(llms, /Do not fund or begin paid community-bounty work yet/);
